...
#
# Verifies:
#   1. seedNewProject seeds 7 baseline skills as project-level rows.
...
echo "→ verifying 7 baseline skills (project-level)"
...
node --input-type=module -e "
  ...
  const expected = ['bitacora-progress', 'framework-detect', 'module-orchestration',
  ...
  if (r.count !== 7) { console.error('✗ expected 7 baseline, got', r.count); process.exit(1); }
"
...
node --input-type=module -e "
  ...
  // Should have 7 baselines + mm-pixel-perfect (NO pb-elements)
  const expectedFolders = [
    'myjarbis-bitacora-progress', 'myjarbis-framework-detect',
...
node --input-type=module -e "
  ...
  if (m.unchanged.length !== 8) { console.error('✗ unchanged should be 8 (7 baselines + mm-pixel-perfect):', m.unchanged); process.exit(1); }
