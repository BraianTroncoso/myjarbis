...
#
# Verifies:
#   1. seedNewProject seeds 6 baseline skills as project-level rows.
...
echo "→ verifying 6 baseline skills (project-level)"
...
node --input-type=module -e "
  ...
  if (r.count !== 6) { console.error('✗ expected 6 baseline, got', r.count); process.exit(1); }
"
...
node --input-type=module -e "
  ...
  // Should have 6 baselines + mm-pixel-perfect (NO pb-elements)
...
node --input-type=module -e "
  ...
  if (m.unchanged.length !== 7) { console.error('✗ unchanged should be 7:', m.unchanged); process.exit(1); }
